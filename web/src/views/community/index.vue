...
import { renderRichText, stripMd } from '../../utils/richtext'
...
const detailCites = computed(() => (detail.value ? parseCitations(detail.value) : []))
  ...
  <div class="page">
      ...
      <div class="col-side">
        <el-card class="card" shadow="never">
          ...
          <div v-if="aiResult" class="ai-result">
            <div class="ai-answer" v-html="renderRichText(aiResult.answer)"></div>
        ...
        <el-card class="card" shadow="never">
          ...
          <div v-loading="loadingHistory" class="history">
            <div
              v-for="h in aiHistory"
              :key="h.id"
              class="his-item"
              role="button"
              tabindex="0"
              @click="openDetail(h)"
              @keydown.enter="openDetail(h)"
              @keydown.space.prevent="openDetail(h)"
            >
              <div class="his-q">{{ h.question }}</div>
              <div class="his-a">{{ stripMd(h.answer) }}</div>
              <div class="his-meta">
                <span class="his-time">{{ (h.created_at || '').replace('T', ' ').slice(0, 16) }}</span>
                <el-button type="danger" link size="small" :loading="deletingId === h.id" @click.stop="doDelete(h.id)">删除</el-button>
              </div>
            </div>
    ...
    >
      <template v-if="detail">
        <div class="d-q">{{ detail.question }}</div>
        <div class="d-time">{{ (detail.created_at || '').replace('T', ' ').slice(0, 16) }}</div>
        <div class="d-label">AI 回答</div>
        <div class="d-a" v-html="renderRichText(detail.answer)"></div>
        <template v-if="detailCites.length">
          <div class="d-label">参考了 {{ detailCites.length }} 条资料</div>
          <div v-for="(c, i) in detailCites" :key="i" class="cite">
            <div class="cite-title">{{ c.title }}</div>
            <div class="cite-snippet">{{ c.snippet }}</div>
          </div>
        </template>
...
/* AI 回答排版：v-html 内容不带 scope，须用 :deep 穿透 */
.d-a :deep(.rt-h),
.ai-answer :deep(.rt-h) {
  margin: 14px 0 6px;
  font-weight: 700;
...
.d-a :deep(.rt-h:first-child),
.ai-answer :deep(.rt-h:first-child) {
  margin-top: 0;
}
.d-a :deep(.rt-p),
.ai-answer :deep(.rt-p) {
...
.d-a :deep(.rt-p:last-child),
.ai-answer :deep(.rt-p:last-child) {
  margin-bottom: 0;
}
.d-a :deep(.rt-list),
.ai-answer :deep(.rt-list) {
  margin: 0 0 10px;
  padding-left: 20px;
}
.d-a :deep(.rt-list li),
.ai-answer :deep(.rt-list li) {
  margin: 4px 0;
}
.d-a :deep(strong),
