...
/** 面试历史列表（含考试记录统计：分数区间 + 每条正确率/答对题数） */
export function listInterviews(page = 1, filters?: InterviewFilters) {
  const params = new URLSearchParams({ page: String(page) })
  if (filters?.minScore != null) params.set('minScore', String(filters.minScore))
  if (filters?.maxScore != null) params.set('maxScore', String(filters.maxScore))
  if (filters?.keyword) params.set('keyword', filters.keyword)
  return http.get(`/interviews?${params.toString()}`) as unknown as Promise<{
    list: InterviewRecord[]
    total: number
    page: number
    pageSize: number
    hasMore: boolean
    stats: { minScore: number | null; maxScore: number | null }
  }>
}

/** 删除一条面试记录（连报告与对话一起删，仅本人） */
export function deleteInterview(id: number | string) {
  return http.delete(`/interviews/${id}`) as unknown as Promise<{ deleted: boolean }>
}
