...
const DEFAULT_POSITION = '国企央企面试';
const DEFAULT_REGION = '四川';
const DEFAULT_TOTAL = 3; // demo 验收：真人对话 3 轮
const IDLE_AUTO_FINISH_MINUTES = 30; // 「进行中」面试超过该时长无作答则自动收尾生成报告
...
async function createInterview(userId, { position, region, totalQuestions } = {}) {
  ...
  if (!questions.length) throw new AppError(3001, '题库暂未配置模拟面试题目，可先前往「历年真题」练习', 404);
...
/** 解析 per_question（mysql2 可能返回字符串或对象） */
function parsePerQuestion(raw) {
  if (!raw) return [];
  if (typeof raw === 'string') {
    try { return JSON.parse(raw); } catch (e) { return []; }
  }
...
/** 正确率：逐题得分 ≥ 60（及格线）算答对，返回整数百分比 */
/** 逐题得分统计：正确率 + 答对题数（及格线 60），无数据返回 null */
function correctStatsOf(rawPer) {
  const arr = parsePerQuestion(rawPer);
  if (!arr.length) return null;
  const pass = arr.filter((pq) => Number(pq.score) >= 60).length;
  return {
    rate: Math.round((pass / arr.length) * 100),
    correctCount: pass,
...
/**
 * 考试记录列表（含筛选：分数区间 + 名称搜索）
 * @returns {Promise<{list:Array, total:number, stats:{minScore,maxScore}}>}
 */
async function listInterviews(userId, { page, pageSize, minScore, maxScore, keyword } = {}) {
  const [base, scoreRange] = await Promise.all([
    sessionModel.findByUser(userId, { page, pageSize, minScore, maxScore, keyword }),
    sessionModel.scoreRange(userId),
  ]);
  const { list, total } = base;

  // 本页已完成记录批量取报告，算正确率/答对题数（避免逐条 N+1）
  const doneIds = list.filter((s) => Number(s.status) === 3).map((s) => s.id);
  const reportRows = doneIds.length ? await reportModel.findBySessions(doneIds) : [];
  const reportMap = new Map(reportRows.map((r) => [r.session_id, r]));

  const enriched = list.map((s) => {
    if (Number(s.status) !== 3) {
      return { ...s, correctRate: null, correctCount: null, totalCount: null };
    }
    const stats = correctStatsOf(reportMap.get(s.id) && reportMap.get(s.id).per_question);
    return {
      ...s,
      correctRate: stats ? stats.rate : null,
      correctCount: stats ? stats.correctCount : null,
      totalCount: stats ? stats.totalCount : null,
    };
  });
  return { list: enriched, total, stats: scoreRange };
}
...
/**
 * 结束面试（统一收尾逻辑）
 * @param {{allowPartial?: boolean}} opts allowPartial=true（超时自动收尾）时：
 *   作答 ≥1 题即可生成报告（题目未答完部分由逐题点评兜底）；0 作答则置为已中断(4) 不生成报告。
 *   完整答完全部题（>= plan.length）才发放面试统计/成长值/勋章，防止部分作答被当作完成一次面试。
 */
async function finalizeSession(sessionId, userId, { allowPartial = false } = {}) {
  ...
  const finished = userAnswers.length >= plan.length;

  if (!allowPartial && !finished) {
    throw new AppError(3001, `面试尚未完成（已完成 ${userAnswers.length}/${plan.length} 题），不能生成报告`, 400);
  }
  if (allowPartial && userAnswers.length === 0) {
    // 一直未作答：直接放弃，置为已中断，不生成报告
    const claimed = await sessionModel.abandonClaim(session.id);
    if (!claimed) {
      // 已被正常结束并发收尾 → 返回现有报告（若有）
      const existing = await reportModel.findBySession(sessionId);
      if (existing) return existing;
    }
    return null;
  ...
  // 幂等收尾：仅当仍处于「进行中」才置为已完成，防止重复收尾/重复加分
  const claimed = await sessionModel.completeClaim(session.id, Math.round(report.totalScore));
  if (!claimed) {
    return reportModel.findBySession(sessionId) || null;
  }
...
/** 正常结束面试：必须完整答完全部题才能生成报告 */
async function endInterview(sessionId, userId) {
  return finalizeSession(sessionId, userId, { allowPartial: false });
}

/** 「进行中」面试超时（默认 30 分钟无作答/无消息）自动收尾扫描：逐条 try/catch，一条失败不影响其它 */
async function autoFinalizeIdleSessions() {
  const idle = await sessionModel.listIdleInProgress(IDLE_AUTO_FINISH_MINUTES);
  const results = [];
  for (const s of idle) {
    try {
      const report = await finalizeSession(s.id, s.user_id, { allowPartial: true });
      results.push({ sessionId: s.id, status: report ? 'completed' : 'interrupted' });
      logger.info(
        report
          ? `面试 ${s.id} 超过 ${IDLE_AUTO_FINISH_MINUTES} 分钟无作答，已自动生成报告`
          : `面试 ${s.id} 超过 ${IDLE_AUTO_FINISH_MINUTES} 分钟无作答且未答题，已置为中断`,
      );
    } catch (e) {
      logger.error(`自动收尾面试 ${s.id} 失败:`, e.message);
    }
  }
  return results;
}
...
function normalizeReport(raw, session, plan, transcript, userId) {
  ...
  // 逐题回顾：题目 × 用户作答 × AI 评分（自动收尾的部分作答：未答到的题计 0 分并标注，避免被当答对）
  ...
  const perQuestion = plan.map((p, i) => {
    ...
    return {
      ...
      answer: answers[i] || '',
      score: answered ? clamp(Math.round(num(rawPer[i] && rawPer[i].score, 60)), 0, 100) : 0,
      comment: answered ? (rawPer[i] && rawPer[i].comment) || '（未生成点评）' : '（超时自动结束，本题未作答）',
    };
  });
