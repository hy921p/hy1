...
/** 文本压成一行并截短（用作标题），超出部分省略 */
function titleOneLine(raw, max = 30) {
  const t = (raw || '').replace(/\s+/g, ' ').trim();
  if (!t) return '';
  return t.length > max ? `${t.slice(0, max)}…` : t;
}

/** AI 摘要存笔记的标题：有来源标题用来源，否则用用户粘贴的句子（压成一行截短），不再用固定的「AI 摘要笔记」 */
function noteTitleFromSummary(summary) {
  return titleOneLine(summary);
}
...
async function aiSummary(userId, { content, sourceType, sourceId, sourceTitle, saveToNote }) {
  ...
  let noteId = null;
  if (saveToNote) {
    // 笔记标题 = 用户粘贴的句子（压成一行截短），正文 = AI 生成的摘要解释 —— 卡片上标题加粗在上、解释在下
    noteId = await noteModel.create({
      userId,
      title: titleOneLine(sourceTitle) || titleOneLine(text) || noteTitleFromSummary(summary),
      content: summary,
