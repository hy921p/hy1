...
const { messageModel, sessionModel } = require('../models/interview');
...
/** 面试历史列表（支持分数区间 + 试卷名称筛选） */
async function list(req, res, next) {
  try {
    const page = parseInt(req.query.page, 10) || 1;
    const pageSize = parseInt(req.query.pageSize, 10) || 10;
    const { minScore, maxScore, keyword } = req.query;
    const { list, total, stats } = await interviewService.listInterviews(req.user.id, {
      page, pageSize, minScore, maxScore, keyword,
    });
    return paginated(res, { list, total, page, pageSize, stats });
  } catch (err) {
    return next(err);
  }
}
...
/** 删除面试记录（连报告与对话消息一起删；仅本人） */
async function remove(req, res, next) {
  try {
    const ok = await sessionModel.removeOwn(Number(req.params.id), req.user.id);
    if (!ok) throw new AppError(1002, '面试记录不存在');
    return success(res, { deleted: true }, '记录已删除');
  } catch (err) {
    return next(err);
  }
}
...
module.exports = { create, list, get, message, end, report, remove };
